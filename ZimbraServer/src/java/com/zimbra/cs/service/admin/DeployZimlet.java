...
public class DeployZimlet extends AdminDocumentHandler {
            ...
            try {
                ...
                if (zf.getZimletDescription().isExtension() && !zsc.getAuthToken().isAdmin()) {
                    throw ServiceException
                            .PERM_DENIED("Only global admin is allowed to deploy extensions for Zimbra Admin UI");
                }
            ...
            if (AdminConstants.A_DEPLOYALL.equals(action)) {
                ...
                for (Server server : servers) {
                    try {
                        ...
                        latch.countDown();
                        ZimbraLog.zimlet.warn("deploy zimlet failed for node %s, coutdown latch %d", server.getName(),
                                latch.getCount(), e);
                    }
                }
                try {
    ...
    public void docRights(List<AdminRight> relatedRights, List<String> notes) {
        ...
        notes.add("If deploying on all servers, need the " + Admin.R_deployZimlet.getName()
                + " right on all servers or on global grant.  If deploying on local server, need " + "the "
                + Admin.R_deployZimlet.getName()
                + " on the local server. Only global admin is allowed to deploy extensions to Zimbra Admin UI.");
    }
}