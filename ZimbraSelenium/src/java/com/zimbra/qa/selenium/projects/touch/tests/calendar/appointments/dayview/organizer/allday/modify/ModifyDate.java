 ...
 * ***** END LICENSE BLOCK *****
 */
package com.zimbra.qa.selenium.projects.touch.tests.calendar.appointments.dayview.organizer.allday.modify;
...
public class ModifyDate extends CalendarWorkWeekTest {
	...
	public void ModifyDate_01() throws HarnessException, ParseException {
		...
		app.zGetActiveAccount().soapSend(
                "<CreateAppointmentRequest xmlns='urn:zimbraMail'>" +
                     "<m>"+
                     	"<inv method='REQUEST' type='event' status='CONF' draft='0' class='PUB' fb='B' transp='O' allDay='1' name='"+ apptSubject +"'>"+
        ...
        app.zPageCalendar.zToolbarPressButton(Button.B_EDIT);
		...
		ZAssert.assertEquals(app.zGetActiveAccount().soapSelectValue("//mail:GetAppointmentResponse//mail:comp//mail:s", "d"), getTomorrowsStartEndDate(), "Verify appointment modified start date");
	    ZAssert.assertEquals(app.zGetActiveAccount().soapSelectValue("//mail:GetAppointmentResponse//mail:comp//mail:e", "d"), getTomorrowsStartEndDate(), "Verify appointment modified end date");

	    // Verify appointment modified time in UI
	 	ZAssert.assertEquals(app.zPageCalendar.zVerifyAppointmentDateInViewAppt(getMovedUTCDay()), true, "Verify modified date shown in view appointment UI");
	}
	...
	public String getTomorrowsStartEndDate() {
	    Date tomorrow = calendarWeekDayUTC.getTime();
