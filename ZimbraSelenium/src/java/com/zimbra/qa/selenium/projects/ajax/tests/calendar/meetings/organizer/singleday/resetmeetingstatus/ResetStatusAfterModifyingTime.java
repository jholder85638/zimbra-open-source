...
public class ResetStatusAfterModifyingTime extends CalendarWorkWeekTest {	
	...
	public void ResetStatusAfterModifyingTime_01() throws HarnessException {
		...
		ZDate changedStartUTC = new ZDate(now.get(Calendar.YEAR), now.get(Calendar.MONTH) + 1, now.get(Calendar.DAY_OF_MONTH) + 1, 10, 0, 0);
		ZDate changedEndUTC   = new ZDate(now.get(Calendar.YEAR), now.get(Calendar.MONTH) + 1, now.get(Calendar.DAY_OF_MONTH) + 1, 12, 0, 0);
