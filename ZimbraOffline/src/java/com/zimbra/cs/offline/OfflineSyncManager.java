...
public class OfflineSyncManager implements FormatListener {
    ...
    public static boolean isAuthError(Exception exception) {
        if (exception instanceof SoapFaultException) {
            String code = ((SoapFaultException)exception).getCode();
            return code.equals(AccountServiceException.AUTH_FAILED) || code.equals(AccountServiceException.TWO_FACTOR_AUTH_FAILED);
        }
