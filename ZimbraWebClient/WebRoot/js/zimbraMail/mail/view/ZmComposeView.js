 ...
 * @param {hash}		bodyOverrides		a hash of body component overrides (optional)
 * @param {String}		extraBodyText		text to prepend to body (overrides user text component TEXT_PRE)
 * @param {Array}		msgIds				list of msg Id's to be added as attachments
 * @param {String}		accountName			on-behalf-of From address
 */
ZmComposeView.prototype.set = function(params) {
	...
	var msg = this._msg = this._origMsg = params.msg,
	    oboMsg = msg || (params.selectedMessages && params.selectedMessages.length && params.selectedMessages[0]),
	    obo = this._getObo(params.accountName, oboMsg);

	if (msg) {
		msg.onChange = this._onMsgDataChange;
	}

    // Convert deprecated argument 'extraBodyText' into an override of ZmComposeView.BC_TEXT_PRE
    params.bodyOverrides = params.bodyOverrides || {};
    var preText = params.bodyOverrides[ZmComposeView.BC_TEXT_PRE] = params.extraBodyText || params.bodyOverrides[ZmComposeView.BC_TEXT_PRE];
    ...
    if (ZmComposeController.IS_REPLY[action]) {
        this._moveCaretOnTimer(preText ? preText.length : 0);
    }
...
function(msg, isDraft, bodyContent) {
	...
	var textContent;
	var content = bodyContent || this._getEditorContent(isDraft);
	...
	if (this._composeMode == Dwt.HTML) {
		...
		if (false && this._htmlEditor) {
			...
			// this.setComponentContent(ZmComposeView.BC_TEXT_PRE, userText)
...
//			this._composeMode = Dwt.HTML;
	...
	msg.textBodyContent = !this._htmlEditor ? textContent : (this._composeMode === Dwt.HTML)
		...
		: this._getEditorContent(isDraft);
...
ZmComposeView.prototype.setComposeMode = function(composeMode, initOnly) {

	if (composeMode === this._composeMode) {
        return;
    }
	...
	var previousMode = this._composeMode,
	    modeSwitch = (!initOnly && previousMode && composeMode && previousMode !== composeMode),
	    userText = modeSwitch && this.getUserText(),
	    quotedText = modeSwitch && this._getQuotedText(),
        overrides = {};
	...
	if (modeSwitch) {
        if (userText) {
            ...
            overrides[ZmComposeView.BC_TEXT_PRE] = userText;
        }
		this.resetBody({
            bodyOverrides:      overrides,
            op:                 htmlMode ? ZmOperation.FORMAT_HTML : ZmOperation.FORMAT_TEXT,
...
function(params) {
	...
	for (var type in params.addrs) {
        var addrs = params.addrs[type];
		var addrs = AjxUtil.toArray(params.addrs[type].list);
		this._recipients.addAddresses(type, AjxVector.fromArray(addrs));
        this._recipients.setAddress(type, params.addrs[type].value || '');
	}
...
ZmComposeView.prototype.reset = function(bEnableInputs) {
    ...
    // used to indicate we've created components out of a saved draft
    this._draftParsed = false;
};
...
ZmComposeView.prototype._getSignatureContentSpan = function(params) {
	...
	if (mode === Dwt.HTML) {
		sigContent = "<div id='" + signatureId + "'>" + sigContent + "</div>";
	}
...
ZmComposeView.prototype._getSignatureSeparator = function(params) {

	var sep = "";
	params = params || {};
	if (params.sigStyle === ZmSetting.SIG_INTERNET) {
		var mode = params.mode || this._composeMode;
		if (mode === Dwt.TEXT) {
			sep += "-- " + this._crlf;
		}
...
ZmComposeView.prototype._setBody = function(params) {
	...
	var incOptions = this._controller._curIncOptions,
	    ac = window.parentAppCtxt || window.appCtxt;

	if (!incOptions) {
		if (ZmComposeController.IS_REPLY[action] && action !== ZmOperation.REPLY_CANCEL) {
    ...
    params.needBody = msg && (isDraft || incOptions.what === ZmSetting.INC_BODY || incOptions.what === ZmSetting.INC_SMART);
	if (params.needBody) {
...
// Zero-width space character we can use to create invisible separators
ZmComposeView.BC_MARKER_CHAR = '\u200B';
// ZmComposeView.BC_MARKER_CHAR = '%';
ZmComposeView.BC_MARKER_REGEXP = {};
ZmComposeView.BC_MARKER_REGEXP[Dwt.TEXT] = new RegExp(ZmComposeView.BC_MARKER_CHAR, 'gi');
ZmComposeView.BC_MARKER_REGEXP[Dwt.HTML] = new RegExp('<div>(' + ZmComposeView.BC_MARKER_CHAR + '+)</div>', 'gi');

// Create a unique marker sequence (vary by length) for each component, and regexes to find them
ZmComposeView.BC_MARKER = {};
ZmComposeView.BC_MARKER[Dwt.TEXT] = {};
ZmComposeView.BC_MARKER[Dwt.HTML] = {};
ZmComposeView.BC_MARKER_COMP = {};
(function() {
    AjxUtil.foreach(ZmComposeView.BC_ALL_COMPONENTS, function(comp, index) {
        if (comp !== ZmComposeView.BC_NOTHING) {
            // Note: relies on BC_NOTHING coming first and using index 0
            var marker = AjxStringUtil.repeat(ZmComposeView.BC_MARKER_CHAR, index);
            ZmComposeView.BC_MARKER[Dwt.TEXT][comp] = marker;
            ZmComposeView.BC_MARKER_COMP[marker] = comp;
			// need to put marker in a DIV since orphaned text is handled by browsers differently
            marker = '<div>' + marker + '</div>';
            ZmComposeView.BC_MARKER[Dwt.HTML][comp] = marker;
        }
    });
})();

ZmComposeView.prototype._setBody1 = function(params) {
	...
	var htmlMode = (this._composeMode === Dwt.HTML),
	    ...
	    incOptions = this._controller._curIncOptions,
        overrides = params.bodyOverrides || {};

	// clear in case of switching from "as attachment" back to "include original message" or to "don't include original"
	this._msgAttId = null;

    var userText = overrides[ZmComposeView.BC_TEXT_PRE] = params.extraBodyText || overrides[ZmComposeView.BC_TEXT_PRE];
	if (userText) {
        // convert text if composing as HTML (check for opening < to see if content is already HTML, should work most of the time)
        if (params.extraBodyTextIsExternal && htmlMode && userText.charAt(0) !== '<') {
            userText = AjxStringUtil.convertToHtml(userText);
        }
        overrides[ZmComposeView.BC_TEXT_PRE] = this._normalizeText(userText);
	}
	...
	if (this._htmlEditor && !params.noEditorUpdate && !isHtmlEditorInitd) {
	...
	var bodyInfo = params.bodyInfo = params.needBody ? this._getBodyContent(msg, htmlMode, incOptions.what) : {};

    // Parse draft content into components in order to recover structure
    if (isDraft && !this._draftParsed) {
        overrides = params.bodyOverrides = this._parseBody(isDraft ? bodyInfo.body : null);
        this._draftParsed = true;
    }

	var value = this._layoutBodyComponents(params);
		
	if (this._htmlEditor && !params.noEditorUpdate) {
	...
	if (isHtmlEditorInitd && !params.noEditorUpdate) {
		this._fixMultipartRelatedImages_onTimer(msg);
	}
    this._saveComponentContent(true);

	var ac = window.parentAppCtxt || window.appCtxt,
	    hasInlineImages = bodyInfo.hasInlineImages || !ac.get(ZmSetting.VIEW_AS_HTML);
...
/**
 * Returns the content of the component within the given body (which defaults to editor content).
 *
 ...
 */
ZmComposeView.prototype.getComponentContent = function(comp, body) {

	if (comp === ZmComposeView.BC_NOTHING) {
		return "";
	}
    body = body || this._getEditorContent(true);
    var components = this._parseBody(body);
    return components[comp];
};

/**
 * Sets the content of the component within the given body (which defaults to editor content).
 *
 * @param {String} comp         component
 * @param {String} compValue    component value
 * @param {String} mode         compose mode
 *
 * @returns {String} revised body content
 */
ZmComposeView.prototype.setComponentContent = function(comp, compValue, mode) {

	var params = {},
		overrides = {};

	params.mode = mode || this._composeMode;
	overrides[comp] = compValue;
	params.bodyOverrides = overrides;

    return this._layoutBodyComponents(params);
};

/**
 * Returns the value of the given component based on data in the user's account.
 * 
 * @param {string}		comp		component identifier (ZmComposeView.BC_*)
 * @param {string}		mode		compose mode
 * @param {hash}		params		msg, include options, overrides, and compose mode
 */
ZmComposeView.prototype.getComponentValue = function(comp, params) {

    params = params || {};
	var mode = params.mode = params.mode || this._composeMode,
        overrides = params.bodyOverrides || {};

    params.action = params.action || this._action;
    params.msg = params.msg || this._msg;
    params.incOptions = params.incOptions || this._controller._curIncOptions;

    // First see if the value was pre-set (from a draft or from text passed in to compose, for example)
	var value = overrides[comp];
	if (value) {
		return value === ZmComposeView.EMPTY ? "" : value;
	}
	else if (params.action === ZmOperation.DRAFT) {
		return "";
	}

	switch (comp) {
		case ZmComposeView.BC_SIG_PRE: {
			return this._getSignatureComponent(ZmSetting.SIG_OUTLOOK, params);
		}
		case ZmComposeView.BC_DIVIDER: {
			return params.needBody ? this._getDivider(params) : '';
		}
		case ZmComposeView.BC_HEADERS: {
			return params.needBody ? this._getHeaders(params) : '';
		}
		case ZmComposeView.BC_QUOTED_TEXT: {
			return params.needBody ? this._getBodyComponent(params) : '';
		}
		case ZmComposeView.BC_SIG_POST: {
			return this._getSignatureComponent(ZmSetting.SIG_INTERNET, params);
        }
	}
};
ZmComposeView.prototype.getComponent = ZmComposeView.prototype.getComponentValue;   // back-compatibility
 ...
 */
ZmComposeView.prototype._layoutBodyComponents = function(params) {
		
    params = params || {};
	params.mode = params.mode || this._composeMode;

    var htmlMode = (params.mode === Dwt.HTML),
        components = ZmComposeView.BC_ALL_COMPONENTS,
	    value = "",
	    prevComp, prevValue;

	for (var i = 0; i < components.length; i++) {
		var comp = components[i];
		var compValue = this.getComponentValue(comp, params) || "";
		...
		if (compValue) {
			compValue = this._getMarker(params.mode, comp) + compValue;
...
ZmComposeView.prototype._getMarker = function(mode, comp) {

    mode = mode || this._composeMode;

    return ZmComposeView.BC_MARKER[mode][comp] || "";
};

// Chart for determining number of blank lines between non-empty components.
ZmComposeView.BC_SPACING = AjxUtil.arrayAsHash(ZmComposeView.BC_ALL_COMPONENTS, function() { return Object() });
...
function(comp1, comp2, val1, val2) {
	...
	val1 = val1 || !!(this.getComponentValue(comp1) || comp1 == ZmComposeView.BC_NOTHING);
	val2 = val2 || !!(this.getComponentValue(comp2) || comp2 == ZmComposeView.BC_NOTHING);
	...
	var incOptions = this._controller._curIncOptions;
	var htmlMode = (this._composeMode === Dwt.HTML);
	if (htmlMode && comp1 === ZmComposeView.BC_DIVIDER && comp2 === ZmComposeView.BC_QUOTED_TEXT && (incOptions.prefix || incOptions.headers)) {
		num = 0;
	}
...
ZmComposeView.prototype._getSignatureComponent = function(style, params) {
		
	var ac = window.parentAppCtxt || window.appCtxt,
        mode = params.mode,
        account = params.account = ac.multiAccounts && this.getFromAccount(),
        value = "";

	if (ac.get(ZmSetting.SIGNATURES_ENABLED, null, account) && ac.get(ZmSetting.SIGNATURE_STYLE, null, account) === style) {
        params.sigStyle = style;
		value = this._getSignatureContentSpan(params);
	}
	return value;
};

ZmComposeView.prototype._getDivider = function(params) {

	var htmlMode = (params.mode === Dwt.HTML),
        msg = params.msg,
	    preface = "";

	if (params.incOptions && params.incOptions.headers) {
		// divider is just a visual separator if there are headers below it
		if (htmlMode) {
			preface = '<hr id="' + AjxStringUtil.HTML_SEP_ID + '">';
		}
        else {
			var msgText = (params.action === ZmOperation.FORWARD_INLINE) ? AjxMsg.forwardedMessage : AjxMsg.origMsg;
		...
		var msgDate = msg.sentDate || msg.date,
		    now = new Date(msgDate),
		    date = AjxDateFormat.getDateInstance(AjxDateFormat.MEDIUM).format(now),
		    time = AjxDateFormat.getTimeInstance(AjxDateFormat.SHORT).format(now),
		    fromAddr = msg.getAddress(AjxEmailAddress.FROM),
		    fromName = fromAddr && fromAddr.getName(),
		    fromEmail = fromAddr && fromAddr.getAddress(),
		    address = fromName;
...
ZmComposeView.prototype._getHeaders = function(params) {

	var htmlMode = (params.mode === Dwt.HTML),
        incOptions = params.incOptions,
	    value = "",
	    headers = [], i, hdr;
    ...
    if (params.msg) {
		for (i = 0; i < ZmComposeView.QUOTED_HDRS.length; i++) {
			hdr = params.msg.getHeaderStr(ZmComposeView.QUOTED_HDRS[i], htmlMode);
			if (hdr) {
				headers.push(hdr);
			}
		}
	...
	if (headers.length) {
		...
		value = this._wrapText(text, htmlMode, incOptions.prefix, true);
	}

	return value;
};

ZmComposeView.prototype._wrapText = function(text, htmlMode, usePrefix, isHeaders) {
	...
	if (usePrefix) {
		wrapParams.prefix = !htmlMode && appCtxt.get(ZmSetting.REPLY_PREFIX);
		if (htmlMode) {
			wrapParams.before = '<div>' + AjxStringUtil.HTML_QUOTE_PREFIX_PRE;
...
ZmComposeView.prototype._getBodyComponent = function(params) {

	var htmlMode = (params.mode === Dwt.HTML),
	    incOptions = params.incOptions,
	    what = incOptions && incOptions.what;

	var bodyInfo = params.bodyInfo || this._getBodyContent(params.msg, htmlMode, what);

	var value = "",
	    body = "";

	if (params.msg && (what === ZmSetting.INC_BODY || what === ZmSetting.INC_SMART)) {
		body = bodyInfo.body;
		// Bug 7160: Strip off the ~*~*~*~ from invite replies.
		if (ZmComposeController.IS_INVITE_REPLY[params.action]) {
			body = body.replace(ZmItem.NOTES_SEPARATOR, "");
		}
		if (htmlMode && body) {
			body = this._normalizeText(body, params.mode);
		}
	}

	body = AjxStringUtil.trim(body);
	if (body) {
		value = this._wrapText(body, htmlMode, incOptions.prefix, false);
	}

	return value;
};

// Removes the invisible markers we use, since we should not send those out as part of the msg
ZmComposeView.prototype._removeMarkers = function(text, mode) {

    mode = mode || this._composeMode;

	return text.replace(ZmComposeView.BC_MARKER_REGEXP[mode], '');
};

ZmComposeView.prototype._normalizeText = function(text, mode) {

    mode = mode || this._composeMode;
    text = this._trimBodyText(text, mode);
    text = this._removeMarkers(text, mode);
    text = text.replace(/\n+$/g, "\n");	// compress trailing line returns

	return AjxStringUtil._NON_WHITESPACE.test(text) ? text + this._crlf : "";
};

ZmComposeView.prototype._trimBodyText = function(text, mode) {
...
ZmComposeView.prototype._saveComponentContent = function(force) {
	...
	var components = ZmComposeView.BC_ALL_COMPONENTS;
	for (var i = 0; i < components.length; i++) {
		var comp = components[i];
...
ZmComposeView.prototype.componentContentChanged = function(comp) {
	return this._compContent && (this._compContent[comp] !== this.getComponentContent(comp));
};

/**
 * Returns text that the user has typed into the editor, as long as it comes first.
 */
ZmComposeView.prototype.getUserText = function() {
    return this.getComponentContent(ZmComposeView.BC_TEXT_PRE);
};

// Returns the block of quoted text from the editor, so that we can see if the user has changed it.
ZmComposeView.prototype._getQuotedText = function() {
 ...
 * @param {String}          op                  operation that resulted in changing body content
 * @param {hash}			bodyOverrides		a hash of body component overrides (optional)
 * @param {hash}			incOptions			include options
 * @param {boolean}			keepAttachments		do not cleanup the attachments
 */
ZmComposeView.prototype.resetBody = function(params) {
    ...
    var action = params.action || this._origAction || this._action,
        op = params.op,
        msg = (this._action === ZmOperation.DRAFT) ? this._origMsg : params.msg || this._origMsg,
        incOptions = params.incOptions || this._controller._curIncOptions;

    var overrides = params.bodyOverrides || this._parseBody();

    if (op === ZmOperation.ADD_SIGNATURE) {
        overrides[ZmComposeView.BC_SIG_PRE] = overrides[ZmComposeView.BC_SIG_POST] = null;
    }
    else if (op === ZmOperation.INCLUDE_HEADERS) {
        overrides[ZmComposeView.BC_HEADERS] = null;
    }
    else if (op === ZmOperation.USE_PREFIX) {
        overrides[ZmComposeView.BC_HEADERS] = null;
        overrides[ZmComposeView.BC_QUOTED_TEXT] = null;
    }
...
/**
 * In text mode, moves the cursor to the beginning of the body.
 *
 * @param {string}  text    beginning text that comes before the cursor
 * @private
 */
ZmComposeView.prototype._setBodyFieldCursor = function(text) {
	...
	if (AjxEnv.isIE) {
		...
		if (text) {
			tr.move('character', text.length + 1);
		}
        else {
			tr.collapse(true);
		}
		...
		var index = text ? text.length + 1 : 0;
...
// During attachment upload, we want to disable certain buttons and menu items. The first call to this function will disable
// any of the given operations and remember which ones we need to re-enable after the upload has finished, which happens on
// the second call.
ZmComposeView.prototype._enableUploadOps = function(enabled) {

	var ops = [ appCtxt.get(ZmSetting.MAIL_SEND_LATER_ENABLED) ? ZmId.OP_SEND_MENU : ZmId.OP_SEND,
				ZmId.OP_DETACH_COMPOSE,
				ZmId.OP_INC_NONE, ZmId.OP_INC_BODY, ZmId.OP_INC_SMART, ZmId.OP_INC_ATTACHMENT, ZmId.OP_USE_PREFIX, ZmId.OP_INCLUDE_HEADERS ],
		toolbar = this._controller._toolbar,
		optionsMenu = toolbar.getButton(ZmId.OP_COMPOSE_OPTIONS).getMenu();

	if (enabled) {
		ops = this._enabledUploadOps;
		appCtxt.notifyZimlets("enableComposeToolbarButtons", [toolbar, true]);
	}
	else {
		var enabledOps = this._enabledUploadOps = [];
	}

	AjxUtil.foreach(ops, function(op) {
		var item = toolbar.getButton(op) || optionsMenu.getMenuItem(op);
		if (item) {
			if (!enabled && item.getEnabled()) {
				enabledOps.push(op);
			}
...
function() {
	...
	var size = Dwt.insetBounds(this.getInsetBounds(), this._htmlEditor.getInsets());
...
// Parses a block of compose text into a hash of components, based on the components being separate by markers.
ZmComposeView.prototype._parseBody = function(body, mode) {
    ...
    var pos = 0, i, j, marker,
        len = body.length,
        ch = ZmComposeView.BC_MARKER_CHAR,
        comp = ZmComposeView.BC_TEXT_PRE,
        regex = ZmComposeView.BC_MARKER_REGEXP[mode],
        components = {};

    if (mode === Dwt.HTML) {
        body = body.replace(/^<html><body><div[^>]+>/i, '');
        body = body.replace(/<\/div><\/body><\/html>$/i, '');
        body = body.replace(regex, "$1");	// strip surrounding DIV from each marker
    }

    while (pos < len) {
        i = body.indexOf(ch, pos);
        var content = body.substring(pos, i === -1 ? len : i);
        components[comp] = this._normalizeText(content, mode);
        if (i === -1) {
            break;
        }
        else {
            j = i + 1;
            while (body.charAt(j) === ch) {
                j++;
            }
            marker = body.substring(i, j);
            comp = ZmComposeView.BC_MARKER_COMP[marker] || comp;
            pos = j;
        }
    }
