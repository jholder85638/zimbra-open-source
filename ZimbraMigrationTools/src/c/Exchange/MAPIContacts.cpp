...
MAPIContactException::MAPIContactException(HRESULT hrErrCode, LPCWSTR lpszDescription, LPCWSTR lpszShortDescription, int nLine, LPCSTR strFile): 
...
HRESULT MAPIContact::Init()
{
    ...
    if (FAILED(hr = HrGetOneProp(m_pMessage, PR_MESSAGE_CLASS, pPropValMsgClass.getptr())))
        throw MAPIContactException(hr, L"Init(): HrGetOneProp Failed.", ERR_MAPI_CONTACT, __LINE__, __FILE__);

    if ((pPropValMsgClass->ulPropTag == PR_MESSAGE_CLASS_W) && (_tcsicmp(pPropValMsgClass->Value.LPSZ, L"ipm.distlist") == 0))
        m_bPersonalDL = true;
    ...
    if (FAILED(hr = m_pMessage->GetIDsFromNames(N_NUM_NAMES, ppNames, MAPI_CREATE,&pContactTags)))
        throw MAPIContactException(hr, L"Init(): GetIDsFromNames Failed.",  ERR_MAPI_CONTACT, __LINE__, __FILE__);
    ...
    // these are the contact properties we need to get. Lower case denotes named props
    SizedSPropTagArray(C_NUM_PROPS, contactProps) = 
    {
        C_NUM_PROPS, 
        {
            ...
            pr_mail3address, pr_mail3entryid, pr_mail3type, pr_mail3dispname, 
            
            pr_fileas, pr_fileasID, PR_DISPLAY_NAME, 
            
            PR_GIVEN_NAME, PR_HOME_ADDRESS_CITY, PR_HOME_ADDRESS_COUNTRY,
    ...
    if (FAILED(hr = m_pMessage->GetProps((LPSPropTagArray) & contactProps, fMapiUnicode, &cVals, &m_pPropVals)))
        throw MAPIContactException(hr, L"Init(): GetProps Failed.", ERR_MAPI_CONTACT, __LINE__, __FILE__);
    ...
    if (m_pPropVals[C_FILEASID].ulPropTag == contactProps.aulPropTag[C_FILEASID])
    {
    ...
    if (m_pPropVals[C_ASSISTANT_TELEPHONE_NUMBER].ulPropTag == contactProps.aulPropTag[C_ASSISTANT_TELEPHONE_NUMBER])
    ...
    if (m_pPropVals[C_COMPANY_MAIN_PHONE_NUMBER].ulPropTag == contactProps.aulPropTag[C_COMPANY_MAIN_PHONE_NUMBER])
        CompanyPhone(m_pPropVals[C_COMPANY_MAIN_PHONE_NUMBER].Value.lpszW);
    
    if (m_pPropVals[C_DEPARTMENT].ulPropTag == contactProps.aulPropTag[C_DEPARTMENT])
        Department(m_pPropVals[C_DEPARTMENT].Value.lpszW);
    
    if (m_pPropVals[C_NICKNAME].ulPropTag == contactProps.aulPropTag[C_NICKNAME])
    ...
    {
        if (zimbraFileAsId == 8 /*OFA_CUSTOM*/)
        {
            ...
            {
                // DCB_BUG_103735 No FileAs property available. Use PR_DISPLAY_NAME instead
                LOG_WARNING(_T("Source contact lacks PR_FILE_AS"));

                LPWSTR pwszSubstituteFileAs = NULL;

                LPWSTR pwszDisplayName = NULL;
                if (PROP_TYPE(m_pPropVals[C_DISPLAY_NAME].ulPropTag) != PT_ERROR)
                    pwszDisplayName = m_pPropVals[C_DISPLAY_NAME].Value.lpszW; 

                if (pwszDisplayName && (wcsicmp(pwszDisplayName, L"") != 0))
                {
                    LOG_WARNING(_T("Using PR_DISPLAY_NAME '%s' in place of PR_FILEAS"), pwszDisplayName);
                    pwszSubstituteFileAs = new WCHAR[wcslen(pwszDisplayName) + 1];
                    wsprintf(pwszSubstituteFileAs, L"%s", pwszDisplayName);
                }
                else
                {
                    pwszSubstituteFileAs = new WCHAR[wcslen(L"NO_NAME") + 1];
                    wsprintf(pwszSubstituteFileAs, L"%s", L"NO_NAME");
                }

                LPWSTR pwszTemp = new WCHAR[wcslen(pwszSubstituteFileAs) + 3];

                // there is a legit string for the custom fileas value
                wsprintf(pwszTemp, L"8:%s", pwszSubstituteFileAs);
                FileAs(pwszTemp);
    ...
    if (m_pPropVals[C_PRIMARY_TELEPHONE_NUMBER].ulPropTag == contactProps.aulPropTag[C_PRIMARY_TELEPHONE_NUMBER])
    ...
    if (m_pPropVals[C_OFFICE2_TELEPHONE_NUMBER].ulPropTag == contactProps.aulPropTag[C_OFFICE2_TELEPHONE_NUMBER])
        ...
        (m_pPropVals[C_MAIL1ADDRESS].ulPropTag == contactProps.aulPropTag[C_MAIL1ADDRESS]))
    {
        ...
        if (wcscmp(m_pPropVals[C_MAIL1TYPE].Value.lpszW, L"EX") == 0)
        {
        ...
        (m_pPropVals[C_MAIL3ADDRESS].ulPropTag == contactProps.aulPropTag[C_MAIL3ADDRESS]))
    {
        ...
        if (wcscmp(m_pPropVals[C_MAIL3TYPE].Value.lpszW, L"EX") == 0)
        {
    ...
    if (m_bPersonalDL)
    {
        ...
        if (m_pPropVals[C_ONEOFFMEMEBRS_IDX].ulPropTag == contactProps.aulPropTag[C_ONEOFFMEMEBRS_IDX])
        {
            bool bFirst = true;
            ...
            for (ULONG i = 0; i < m_pPropVals[C_ONEOFFMEMEBRS_IDX].Value.MVbin.cValues; i++)
            {
                ...
                hr = m_session->OpenEntry(bin.cb, (LPENTRYID)bin.lpb, NULL, 0, &dwObjType, (LPUNKNOWN *)&pUser);
                if (FAILED(hr) || !pUser)
                    continue;
                ...
                if (pDNProp && (pDNProp->ulPropTag == PR_DISPLAY_NAME_W) && wcslen(pDNProp->Value.lpszW))
                    user += pDNProp->Value.lpszW;

                if (pAddrProp && (pAddrProp->ulPropTag == PR_EMAIL_ADDRESS_W) && wcslen(pAddrProp->Value.lpszW))
                {
    ...
    if (!m_bPersonalDL)
    {
        ...
        if (hr == S_OK)
        {
            hr = m_pMessage->GetNamesFromIDs(&tagArray, (LPGUID)&PS_PUBLIC_STRINGS, 0, &lCount, &lppPropNames);
            if (SUCCEEDED(hr))
            {
                ...
                for (ULONG i = 0; i < lCount; i++)
                {
                    ...
                    // Bug 87322. Minimize the number of calls to HrGetOneProp
                    if ((lppPropNames[i] != NULL) && lppPropNames[i]->ulKind == MNID_STRING)
                    {
                        hr = HrGetOneProp(m_pMessage, tagArray->aulPropTag[i], pPropVal.getptr());
                        if ((hr==S_OK) &&(pPropVal.getptr()!=NULL))   // FBS bug 71786 -- 3/20/12
                        {
...
HRESULT MAPIContact::GetContactImage(wstring &wstrImagePath,wstring &wstrContentType,wstring &wstrContentDisposition)
{
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                for (unsigned int i = 0; i < pAttachRows->cRows; i++)
                {
                    ...
                    if (_tcscmp(pAttachRows->aRow[i].lpProps[2].Value.LPSZ, _T("ContactPicture.jpg")))
                        continue;

                    Zimbra::Util::ScopedInterface<IAttach> pAttach;
                    if (FAILED(hr = m_pMessage->OpenAttach(  pAttachRows->aRow[i].lpProps[0].Value.l, NULL, 0, pAttach.getptr())))
                        continue;
                    if (FAILED(hr = pAttach->OpenProperty(PR_ATTACH_DATA_BIN, &IID_IStream, STGM_READ, 0, (LPUNKNOWN FAR *)pSrcStream.getptr())))
                        return hr;
    ...
    // Open stream on file
    if (FAILED(hr = OpenStreamOnFile(MAPIAllocateBuffer, MAPIFreeBuffer, STGM_CREATE | STGM_READWRITE, (LPTSTR)strFQFileName.c_str(), NULL, pDestStream.getptr())))
        return hr;
