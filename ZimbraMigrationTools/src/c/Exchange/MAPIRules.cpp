...
CRule::CRule(): 
    m_bProcessAdditionalRules(true), 
    m_lActive(false), 
    m_bOrProcessed(false),
    m_bToOrCc(false), 
    m_bNotOr(false) 
{
    LOGFN_TRACE_NO;
 ...
 */
BOOL ConditionNameToString(RuleCondition condition, RuleConditionOp conditionOp, std::wstring &wstrName)
{
    LOGFN_TRACE_NO;
                                              ...
                                              ULONG ulConditionVal, BOOL bNegative)
{
    ...
    if (pwzConditionOp)
    {
        ...
        if (0 == lstrcmpiW(pwzConditionOp, L"contains"))
            return (bNegative) ? DoesNotContain : Contains;
        else
        if (0 == lstrcmpiW(pwzConditionOp, L"under"))
            return (bNegative) ? NotUnder : Under;
        else 
        if (0 == lstrcmpiW(pwzConditionOp, L"over"))
            return (bNegative) ? NotOver : Over;
        else 
        if (0 == lstrcmpiW(pwzConditionOp, L"before"))
            return (bNegative) ? NotBefore : Before;
        else 
        if (0 == lstrcmpiW(pwzConditionOp, L"after"))
            return (bNegative) ? NotAfter : After;
        else 
        if (0 == lstrcmpiW(pwzConditionOp, L"matches"))
            return (bNegative) ? DoesNotMatchPattern : MatchesPattern;
        else 
        if (0 == lstrcmpiW(pwzConditionOp, L"in"))
            return (bNegative) ? NotIn : In;
    }

    if (pwzConditionName)
    {
        if (   (0 == lstrcmpiW(pwzConditionName, L"attachment")) 
            || (0 == lstrcmpiW(pwzConditionName, L"exists")))
        {
            return (bNegative) ? DoesNotExist : Exists;
        }
        else 
        if (0 == lstrcmpiW(pwzConditionName, L"invite"))
        {
...
BOOL ConditionDetailsToInfo(LPCWSTR pwzTestName, LPCWSTR pwzConditionName, 
                            LPCWSTR pwzConditionComparison, LPCWSTR pwzTest, LPCWSTR pwzValue, BOOL bNegative,
                            RuleConditionInfo &ruleConditionInfo)
{
    ...
    ruleConditionInfo.m_ruleCondition   = ConditionFromString(pwzConditionName, ruleConditionInfo.m_wstrTest);
    ruleConditionInfo.m_ruleConditionOp = ConditionComparisonFromString(pwzConditionComparison, pwzConditionName, ruleConditionInfo.m_ruleCondition, bNegative);
...
{
    LOGFN_TRACE_NO;
    ConditionNameToString(ruleConditionInfo.m_ruleCondition, ruleConditionInfo.m_ruleConditionOp, wstrConditionName);
...
CRuleProcessor::CRuleProcessor(Zimbra::MAPI::MAPISession* session, Zimbra::MAPI::MAPIStore* store, std::wstring account) : 
    m_session(session), 
    m_userStore(store), 
    m_account(account) 
{
...
//
// Recurses back up the folder hierarchy from the passed in folder, getting the parent folder
// names and building the path
//
{
    LOGFN_TRACE_NO;
    ...
    hr = m_userStore->OpenEntry(lpParentEID->Value.bin.cb, (LPENTRYID)lpParentEID->Value.bin.lpb, NULL, 0, &ulObjType, (LPUNKNOWN *)&pParentFolder);
    if (FAILED(hr))
        return vFolders[0];
    ...
    hr = HrGetOneProp(m_userStore->GetInternalMAPIStore(), PR_IPM_SUBTREE_ENTRYID, spIPMSubtreeEntryId.getptr());
    if (FAILED(hr))
        return vFolders[0];
    ...
    {
        // Not reached the top folder yet -> recurse
        wstring sDelocalizedFolderName = GetDelocalizedFolderName(pParentFolder, lpParentEID->Value.bin.cb, (LPENTRYID)(lpParentEID->Value.bin.lpb));

        vFolders.push_back(sDelocalizedFolderName.c_str());

        return MakeFolderPath(pParentFolder, vFolders);
    }
}

bool CRuleProcessor::ProcessRestrictions(CRule &rule, LPSRestriction pRestriction, bool bNegative, ULONG ulCallingType)
{
    LOGFN_TRACE_NO;
    bool bRetval = true;
    ...
    {
        LOG_WARNING(_T("NULL Restriction"));
        return false;
    }
    ...
    switch (pRestriction->rt)
    {
        case RES_CONTENT:
        {
            ...
            switch (PROP_ID(pProp->ulPropTag))
            {
                case PROP_ID(PR_SUBJECT):
                {
                    ...
                    ConditionDetailsToInfo(wTestName, wName, wComparison, wTest, wValue, bNegative, ruleConditionInfo);
                ...
                case PROP_ID(PR_MESSAGE_CLASS):
                {
                    ...
                    if (strcmp(pProp->Value.lpszA, "IPM.Note.Rules.OofTemplate.Microsoft") == 0)
                    {
                        LOG_WARNING(_T("'which is an automatic reply' condition not supported"));
                        rule.SetActive(0);
                    }
                    else 
                    if (strncmp(pProp->Value.lpszA, "IPM.Note.", 9) == 0)
                    {
                        LOG_WARNING(_T("'uses the form' condition not supported"));
                        rule.SetActive(0);
                    }
                    break;
                }
                ...
                default:
                    //LOG_WARNING("Restriction content property", pProp->ulPropTag, "not supported");
                    LOG_WARNING(_T("Restriction content property not supported"));
                    rule.SetActive(0);
            }
            break;
        }
        ...
        case RES_PROPERTY:
        {
            ...
            switch (PROP_ID(pProp->ulPropTag))
            {
                case PROP_ID(PR_MESSAGE_SIZE):
                {
                    ...
                    wcscat(wSizval, L"K");
                    ConditionDetailsToInfo(wTestName, wName, wComparison, wTest, wSizval, bNegative, ruleConditionInfo);
                    rule.AddCondition(ruleConditionInfo);
                    break;
                }
                ...
                case PROP_ID(PR_MESSAGE_DELIVERY_TIME):
                {
                    ...
                    ConditionDetailsToInfo(wTestName, wName, wComparison, wTest, wDateval, bNegative, ruleConditionInfo);
                    rule.AddCondition(ruleConditionInfo);
                    break;
                }
                ...
                {
                    LOG_WARNING(_T("'flagged for action' %s not supported"), wMode);
                    rule.SetActive(0);
                    break;
                }
                ...
                case PROP_ID(PR_MESSAGE_CLASS):
                {
                    LOG_WARNING(_T("'RSS Feed' %s not supported"), wMode);
                    rule.SetActive(0);
                    break;
                }
                ...
                {
                    LOG_WARNING(_T("'sent only to me' %s not supported"), wMode);
            ...
            switch (ulPropTag)
            {
                case PR_MESSAGE_FLAGS:
                {
                    if (pRestriction->res.resBitMask.ulMask == MSGFLAG_HASATTACH)
                    {
                        ...
                        ConditionDetailsToInfo(wTestName, wName, NULL, NULL, NULL, bNegative, ruleConditionInfo);
                        rule.AddCondition(ruleConditionInfo);
                    }
                    break;
                }
        ...
        case RES_AND:
        {
            ...
            for (i = 0; i < ulCount; i++)
            {
                if (!ProcessRestrictions(rule, &pRestriction->res.resAnd.lpRes[i], bNegative, RES_AND))
        ...
        case RES_OR:
        {
            ...
            for (i = 0; i < ulCount; i++)
            {
                if (!ProcessRestrictions(rule, &pRestriction->res.resOr.lpRes[i], bNegative, RES_OR))
                    bRetval = false;                // if any one fails, we want to log them all, but not continue
            }
            break;
        }
        ...
        case RES_NOT:
        {
            ...
            {
                LOG_WARNING(_T("mixed OR and AND conditions not supported"));
                bRetval = false;
            }
        ...
        case RES_SUBRESTRICTION:
        {
            ...
            if (ulSubObject == PR_MESSAGE_RECIPIENTS)
                ProcessRestrictions(rule, pRestriction->res.resSub.lpRes, bNegative, RES_SUBRESTRICTION);
            break;
        }
        ...
        case RES_EXIST:
        {
            ...
            {
                LOG_WARNING(_T("Client-only %s not yet supported.  Possible conditions causing this:"), wMode);
                LOG_WARNING(_T("   sender is in specified address book"));
                LOG_WARNING(_T("   through specified account"));
                LOG_WARNING(_T("   assigned to category"));
                LOG_WARNING(_T("   with selected properties of documents of forms"));
                LOG_WARNING(_T("   with specific words in the recipient's address"));
                LOG_WARNING(_T("   no conditions specified"));
                rule.SetActive(0);
            }
            else if (PROP_ID(ulPropTag) == 0x8008)
            {
                LOG_WARNING(_T("'assigned to any category' %s not supported"), wMode);
                rule.SetActive(0);
            }
            break;
        }
        ...
        default:
            LOG_WARNING(_T("Restriction type %d not supported"), pRestriction->rt);
            rule.SetActive(0);
    } // switch
    return bRetval;
}
...
wstring CRuleProcessor::GetDelocalizedFolderName(LPMAPIFOLDER pFolder, ULONG cbFldEntryId, LPENTRYID lpFldEntryId)
{
    ExchangeSpecialFolderId efid = EXSFID_NONE;
    _ASSERT(m_session);
    _ASSERT(m_userStore);

    // Get folder name
    Zimbra::Util::ScopedBuffer<SPropValue> pPropDisplayName;
    HrGetOneProp(pFolder, PR_DISPLAY_NAME, pPropDisplayName.getptr());
    if (!pPropDisplayName.get())
    {
        LOG_ERROR(_T("Unable to read folder name"));
        return L"";
    }
    wstring sFolderName = pPropDisplayName->Value.lpszW;

    // We've got the folder name, but if its a special folder, it might be localized.
    // -> delocalize it using GetExchangeSpecialFolderId()
    ...
    wstring sUnlocalizedSFName;
    efid = Zimbra::MAPI::Util::GetExchangeSpecialFolderId(m_userStore->GetInternalMAPIStore(), cbFldEntryId, lpFldEntryId, &specialFolderIds);
    // efid is the index in the array "specialFolderIds"
    switch (efid)
    {
        case EXSFID_NONE:
            LOG_GEN(_T("Not a special folder"));
            break;
        case EXSFID_INBOX:
            sUnlocalizedSFName = _T("Inbox");
            break;
        case EXSFID_CALENDAR:
            sUnlocalizedSFName = _T("Calendar");
            break;
        case EXSFID_CONTACTS:
            sUnlocalizedSFName = _T("Contacts");
            break;
        case EXSFID_DRAFTS:
            sUnlocalizedSFName = _T("Drafts");
            break;
        case EXSFID_TASK:
            sUnlocalizedSFName = _T("Tasks");
            break;
        case EXSFID_OUTBOX:
            sUnlocalizedSFName = _T("Outbox");
            break;
        case EXSFID_SENTMAIL:
            sUnlocalizedSFName = _T("Sent");
            break;
        case EXSFID_TRASH:
            sUnlocalizedSFName = _T("Trash");
            break;
        case EXSFID_JUNK_MAIL:
            sUnlocalizedSFName = _T("Junk");
            break;
        /*
        case EXSFID_JOURNAL:
            sUnlocalizedSFName = _T("Journal");
            break;
        case EXSFID_NOTE:
            sUnlocalizedSFName = _T("Note");
            break;
        case EXSFID_SYNC_CONFLICTS:
            sUnlocalizedSFName = _T("Trash");
            break;
        case EXSFID_SYNC_ISSUES:
            sUnlocalizedSFName = _T("Trash");
            break;
        case EXSFID_SYNC_LOCAL_FAILURES:
            sUnlocalizedSFName = _T("Trash");
            break;
        case EXSFID_SYNC_SERVER_FAILURES:
            sUnlocalizedSFName = _T("Trash");
            break;
        case EXSFID_SUGGESTED_CONTACTS:
            sUnlocalizedSFName = _T("Trash");
            break;
        */
        default:
    ...
    // Update sFolderName with the delocalized version - if we found one
    if (   (sUnlocalizedSFName != L"")
        && (sUnlocalizedSFName != sFolderName))
    {
        LOG_TRACE(_T("Delocalizing folder name %s -> %s"), sFolderName.c_str(), sUnlocalizedSFName.c_str());
        sFolderName = sUnlocalizedSFName;
    }
    ...
    for (i = 0; i < pActions->cActions; i++)
    {
        ...
        switch (action.acttype)
        {
            ...
            case OP_MOVE:
            {
                ...
                hr = m_session->OpenEntry(cbFldEntryId, lpFldEntryId, NULL, MAPI_BEST_ACCESS, &objtype, (LPUNKNOWN *)&pFolder);
                if (hr == S_OK)
                {
                    ruleAction.m_ruleAction = RuleActionFromString(L"fileInto");

                    // DCB_BUG_101833 Any special folders in "fileinto" need to be non-localized.
                    // When migrating from localized exchange account, special folder names will be localized, so 
                    // grab non-localized name now.
                    wstring sFolderName = GetDelocalizedFolderName(pFolder, cbFldEntryId, lpFldEntryId);

                    if (sFolderName != L"Top of Information Store")
                    {
                        // Recurse back up through parents to build up full folder path
                        std::vector<CString> vFolders;
                        vFolders.push_back(sFolderName.c_str());
                        ruleAction.m_wstrArg = MakeFolderPath(pFolder, vFolders);
                    }
                    else
                    {
                        LOG_ERROR(_T("Unable to determine folder path.folderpath pointing to top of information store"));
                        rule.SetActive(0);
                    }
                ...
                {
                    LOG_WARNING(_T("Unable to determine folder path"));
                    rule.SetActive(0);
                }
                break;
            }
            ...
            case OP_COPY:
            {
                LOG_WARNING(_T("'move a copy to the specified folder' action is not supported"));
                rule.SetActive(0);
                break;
            }
            ...
            case OP_FORWARD:
            {
                ...
                {
                    LOG_WARNING(_T("'redirect' action is not supported"));
                    rule.SetActive(0);
                }
                else if (action.ulActionFlavor == 4)
                {
                    LOG_WARNING(_T("'forward as an attachment' action is not supported"));
                    rule.SetActive(0);
                }
                else
                {
                    for (ULONG j = 0; j < action.lpadrlist->cEntries; j++)
                    {
                        ...
                        hr = m_session->OpenEntry(cbEID, lpEID, NULL, MAPI_BEST_ACCESS, &objtype, (LPUNKNOWN *)spUnk.getptr());
                        if (hr == S_OK)
                        {
                            ...
                            SizedSPropTagArray(2, propTags) = {2, { PR_DISPLAY_NAME_W, PR_EMAIL_ADDRESS_W }};
                            ULONG cVals = 0;
                            hr = spUnk->GetProps((LPSPropTagArray) & propTags, fMapiUnicode, &cVals, pPropVals.getptr());
                            if (pPropVals[0].ulPropTag == PR_DISPLAY_NAME_W)
                                ruleAction.m_wstrArg = pPropVals[0].Value.lpszW;
                            else 
                            if (pPropVals[1].ulPropTag == PR_EMAIL_ADDRESS_W)
                        ...
                        {
                            LOG_WARNING(_T("Unable to determine address"));
                            rule.SetActive(0);
                        }
                    }
                }
            ...
            case OP_REPLY:
            {
                LOG_WARNING(_T("'have server reply using a specific message' action is not supported"));
                rule.SetActive(0);
                break;
            }
            ...
            case OP_TAG:
            {
                LOG_WARNING(_T("'clear message\'s categories' action is not supported"));
                rule.SetActive(0);
                break;
            }
            ...
            {
                BYTE AssignToCategory[14] = { 0x01, 0x80, 0x33, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
                BYTE PermDelete[10]       = { 0x01, 0x80, 0x4a, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
                BYTE MarkAsRead[10]       = { 0x01, 0x80, 0x4C, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
                BYTE ClearMFlag[10]       = { 0x01, 0x80, 0x32, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
                ...
                if (cbData > 0)
                {
                    ...
                    if (dataPtr = memmem(lpData, cbData, L"Follow up", 9))
                    {
                        ...
                        if (dataPtr[0] == 0x51)     // make sure it's really a Follow up
                        {
                            ...
                            if (dataPtr[0] == 0x01) // Today
                            {
                                ...
                                rule.AddAction(ruleAction);
                                LOG_WARNING(_T("Deferred action 'flag message'.  If there are other non-deferred actions on this rule, you must re-add them."));
                            }
                            else
                            {
                                LOG_WARNING(_T("Follow up actions other than today are not supported"));
                            }
                        }
                    }
                    ...
                    {                               // right after the memory block we check for, the length of the category name is stored, followed by the name.
                        ...
                        rule.AddAction(ruleAction);
                        delete[] pWTagname;
                        LOG_WARNING(_T("Deferred action 'assign to category'.  If there are other non-deferred actions on this rule, you must re-add them."));
                    }
                    if (memmem(lpData, cbData, PermDelete, 10))     // hokey
                    {
                        ...
                        rule.AddAction(ruleAction);
                        LOG_WARNING(_T("Deferred action 'permanently delete it'.  If there are other non-deferred actions on this rule, you must re-add them."));
                    }
                    if (memmem(lpData, cbData, MarkAsRead, 10))     // hokey
                    {
                        ...
                        rule.AddAction(ruleAction);
                        LOG_WARNING(_T("Deferred action 'mark as read'.  If there are other non-deferred actions on this rule, you must re-add them."));
                    }
                    if (memmem(lpData, cbData, ClearMFlag, 10))     // hokey
                    {
                        bRecognize = true;
                        LOG_WARNING(_T("clear message flag action not supported"));
                        rule.SetActive(0);
                    }
                    if (memmem(lpData, cbData, PlayASound, 6))      // hokey
                    {
                        bRecognize = true;
                        LOG_WARNING(_T("play a sound action not supported"));
                        rule.SetActive(0);
                    }
                    if (!bRecognize)
                    {
                        LOG_WARNING(_T("This client-only rule is not supported"));
                        bRetval = false;
                    }
                }
                else
                {
                    LOG_WARNING(_T("This client-only rule is not supported -- internal processing error"));
                    bRetval = false;
                }
                break;
            }
            ...
            default:
                LOG_WARNING(_T("Action type not supported"));
                bRetval = false;
        } // switch
    }
...
LPBYTE CRuleProcessor::memmem(const void *buf, size_t buf_len, const void *byte_sequence, size_t byte_sequence_len)
{
    BYTE *bf = (BYTE *)buf;
...
void CRuleMap::WriteFilterTests(CRule &rule, LPWSTR &filterTests)
{
        ...
        conditionIndex++)
    {
        ...
        ConditionInfoToDetails(ruleConditionInfo, wstrConditionName, wstrOpName, wstrTestName, wstrTest, wstrValue, bNegative);
        ...
        if (ruleConditionInfo.m_ruleCondition == HeaderNamed)
        {
            ...
            if (ruleConditionInfo.m_ruleConditionOp == DoesNotExist)
        ...
        if (0 == lstrcmpiW((LPCWSTR)wstrTestName.c_str(), L"addressBookTest"))
        {
            ...
            if (0 == lstrcmpiW((LPCWSTR)wstrTest.c_str(), L"Cc"))
                wstrHdrTest = L"cc";
            else 
            if (0 == lstrcmpiW((LPCWSTR)wstrTest.c_str(), L"Bcc"))
                wstrHdrTest = L"bcc";
